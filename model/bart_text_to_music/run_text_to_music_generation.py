...
class ModelArguments:
    ...
    patience: Optional[int] = field(default=None, metadata={"help": ""})
...
class DataTrainingArguments:
    ...
    melody_column: Optional[str] = field(
        default=None,
        metadata={
            "help":
    ...
    test_file: Optional[str] = field(
        ...
        metadata={
            "help":
                "An optional input test data file to evaluate the metrics on (a jsonlines or csv file)."
        },
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        # for deepspeed
        config_json_filename = os.path.abspath(sys.argv[2])
        model_args, data_args, training_args = parser.parse_json_file(
            json_file=os.path.abspath(config_json_filename))
    else:
    ...
    # summaries (unless you specify column names for this with the `text_column` and `melody_column` arguments).
    #
    ...
    if data_args.melody_column is None:
        melody_column = dataset_columns[
            1] if dataset_columns is not None else column_names[1]
    else:
        melody_column = data_args.melody_column
        if melody_column not in column_names:
            raise ValueError(
                f"--melody_column' value '{data_args.melody_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[text_column])):
            if examples[text_column][i] and examples[melody_column][i]:
                inputs.append(examples[text_column][i])
                targets.append(examples[melody_column][i])
    ...
    trainer = Seq2SeqTrainer(
        ...
        callbacks=[
            transformers.EarlyStoppingCallback(
                early_stopping_patience=model_args.patience)
        ])
    ...
    if training_args.do_predict:
        # FIXME evaluationと一緒に行うとgpuのoomになる
        logger.info("*** Predict ***")

        losses = []
        if training_args.keep_loss:
            for step, inputs in enumerate(
                    trainer.get_test_dataloader(test_dataset=predict_dataset)):
                with torch.no_grad():
                    has_labels = False if len(
                        trainer.label_names) == 0 else all(
                        inputs.get(k) is not None
                        for k in trainer.label_names)
                    ...
                    loss_without_labels = True if len(
                        trainer.label_names) == 0 and return_loss else False

                    inputs = trainer._prepare_inputs(inputs)

                    if hasattr(trainer.model, "config"):
                        ignore_keys = getattr(trainer.model.config,
                    ...
                    if has_labels or loss_without_labels:
                        labels = nested_detach(
                            tuple(
                                inputs.get(name)
                                for name in trainer.label_names))
                        if len(labels) == 1:
                            labels = labels[0]
                    else:
                        labels = None
                    if has_labels or loss_without_labels:
                        with trainer.compute_loss_context_manager():
                            ...
                            masked_lm_loss = loss_fct(
                                lm_logits.view(-1, model.config.vocab_size),
                                labels.view(-1)).view(
                                labels.shape[0],
                                -1).detach().mean(axis=1).detach().cpu()
                            losses.append(masked_lm_loss.cpu())
        output_loss_file = os.path.join(training_args.output_dir,
                                        "prediction_losses.npz")
        output_source_path_file = os.path.join(training_args.output_dir,
                                               "loss_source_paths.txt")
        with open(output_source_path_file, 'w') as f:
            f.write('\n'.join(raw_datasets['test']['source_path']) + '\n')

        losses = torch.concat(losses, dim=0).numpy()
        print(losses.shape)
        numpy.savez(output_loss_file, losses)
        # writer.write("\n".join(predictions))
